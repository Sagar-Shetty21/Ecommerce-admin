...
const Size = ({setInactive, setVarients, varients}) => {
  ...
  const handleAddVarient = (e) => {
    e.preventDefault();
    setVarients(prev => {
        return [...prev,{sizeInfo, price}]
    })
    setSizeInfo("");
    setPrice("");
  }

  const deleteSizeVarient = (indexToRemove) => {
  ...
  return (
    <div className="size-details-input-container">
        ...
        <form className="new-varient-input-container" onSubmit={(e) => handleAddVarient(e)}>
            <input type="text" placeholder="size info" required value={sizeInfo} onChange={(e) => setSizeInfo(e.target.value)} />
            ...
            {varients.map((item, index) => {
                return (
                    <div className="size-varient-card" key={index}>
                        <span className="size">{item.sizeInfo}</span>
                        <span className="price">₹{item.price}</span>
                        <div className="remove-varient-btn" onClick={() => deleteSizeVarient(index)}>
